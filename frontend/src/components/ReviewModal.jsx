...
export default function ReviewModal({ isOpen, setOpen, hostel }) {
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      ...
      formData.append("photos", file); // Append the file to formData
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        "https://automatic-info-tracker-backend.onrender.com/service/add_review/",
        {
          method: "POST",
          body: formData,
        }
    ...
    <MDBModal open={isOpen} setOpen={setOpen} tabIndex="-1">
      <MDBModalDialog centered scrollable size="lg">
        <MDBModalContent>
          ...
          <MDBModalFooter>
            <MDBBtn color="secondary" onClick={handleSubmit}>
              Submit Review
            </MDBBtn>
            {submittedReview && (
              <div>
